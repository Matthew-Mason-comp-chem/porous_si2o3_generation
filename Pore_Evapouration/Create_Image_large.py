...
class DrawLineWidget(object):
    def __init__(self,):
        """
        Replacement DrawLineWidget.__init__
        - ensures nodes/dual are loaded
        - creates a tight canvas focused on the data
        - sets up mouse callback and initial state
        """
        # preserve previous attributes where appropriate
        self.new_ring = 0
        self.folder = "Results"

        # ensure nodes/dual are loaded
        self.lj = 1
        ...
        # default image placeholders (we'll replace with generated canvas)
        self.original_image = None
        self.clone = None

        # mouse interaction
        cv2.namedWindow('image', cv2.WINDOW_NORMAL)
        # initial window size (user can resize)
        try:
            cv2.resizeWindow('image', 1200, 800)
        except Exception:
            # some backends ignore resize; that's OK
            pass

        # List to store start/end points for interactions
        self.image_coordinates = []

        # --- Build a tight canvas tuned to give many pixels to nodes/edges ---
        # collect coordinates safely
        all_x, all_y = [], []
        for n in self.nodes.values():
            cr = np.asarray(n.get('crds', [0.0, 0.0]), dtype=float)
            all_x.append(float(cr[0])); all_y.append(float(cr[1]))
        for d in self.dual.values():
            cr = np.asarray(d.get('crds', [0.0, 0.0]), dtype=float)
            all_x.append(float(cr[0])); all_y.append(float(cr[1]))
        ...
        # very small margin so canvas is tight around data (avoids huge white whitespace)
        world_margin = 0.02 * max(max_x - min_x, max_y - min_y) if max_x > min_x else 1.0
        world_width = (max_x - min_x) + 2 * world_margin
        world_height = (max_y - min_y) + 2 * world_margin

        # compute average neighbor (edge) distance in world units
        dists = []
        for key, n in self.nodes.items():
            cr = np.array(n.get('crds', [0.0, 0.0]), dtype=float)
            for nb in n.get('net', []):
                try:
                    nbcr = np.array(self.nodes[str(int(nb))]['crds'], dtype=float)
                    d = np.linalg.norm(cr - nbcr)
                    if d > 0:
                        dists.append(d)
                except Exception:
                    pass
        avg_dist = float(np.mean(dists)) if len(dists) else 1.0

        # choose desired pixels per average edge so nodes actually get many pixels
        desired_edge_px = 90   # << tweak this up (e.g. 120) if you want even larger nodes/edges

        # compute scale (pixels per world unit)
        self.scale = int(max(1, desired_edge_px / avg_dist))

        # build a tight canvas around the world
        pad_px = 120   # small border in pixels
        canvas_w = int(self.scale * world_width) + 2 * pad_px
        canvas_h = int(self.scale * world_height) + 2 * pad_px

        # clamp canvas dims to avoid runaway memory use
        MAX_DIM = 16000   # adjust upward only if you have plenty of RAM
        if canvas_w > MAX_DIM or canvas_h > MAX_DIM:
            # reduce scale to fit within MAX_DIM (preserves relative sizes)
            scale_x = max(1, int(MAX_DIM / (world_width + 1e-9)))
            scale_y = max(1, int(MAX_DIM / (world_height + 1e-9)))
            self.scale = min(self.scale, scale_x, scale_y)
            canvas_w = min(canvas_w, MAX_DIM)
            canvas_h = min(canvas_h, MAX_DIM)

        # recompute offsets so world is placed nicely inside canvas
        self.x_offset = int(pad_px + self.scale * (-min_x + world_margin))
        self.y_offset = int(pad_px + self.scale * (-min_y + world_margin))

        # create white canvas and initial clone
        self.original_image = np.ones((canvas_h, canvas_w, 3), dtype=np.uint8) * 255
        self.clone = self.original_image.copy()
        self._canvas_size = (canvas_w, canvas_h)

        # register mouse callback (keeps your existing interactive behavior)
    ...
    def refresh_new_cnxs(self, atoms):
        """
        Replacement refresh_new_cnxs(self, atoms)
        - updates networkx attributes and redraws the whole canvas
        - uses explicit pixel radii for nodes/rings and thicker lines for edges
        """
        # update the Nodes/Dual graphs or node positions as your original code expects
        try:
            self.update_Nodes()
        except Exception:
            # fallback: if update_Nodes doesn't exist or fails, continue with existing self.Nodes/self.Dual
            pass

        # start from a fresh copy of the base canvas
        if self.original_image is None:
            # safety: create a small default canvas if init didn't run
            self.original_image = np.ones((1000, 1000, 3), dtype=np.uint8) * 255
        self.clone = self.original_image.copy()

        # get node and ring positions (from networkx)
        node_pos = nx.get_node_attributes(self.Nodes, 'pos')
        ring_pos = nx.get_node_attributes(self.Dual, 'pos')

        # explicit pixel radii for readability (floor values ensure minimum visible size)
        node_px = max(6, int(0.12 * self.scale))   # pixel radius for nodes (tweak if needed)
        ring_px = max(5, int(0.08 * self.scale))   # pixel radius for rings

        # draw filled nodes with outline
        for node in node_pos:
            cx = int(self.scale * node_pos[node][0] + self.x_offset)
            cy = int(self.scale * node_pos[node][1] + self.y_offset)
            # filled circle (red)
            cv2.circle(self.clone, (cx, cy), node_px, (255, 0, 0), -1)
            # thin black outline
            cv2.circle(self.clone, (cx, cy), node_px, (0, 0, 0), 1)

        # draw rings as filled green with outline
        for ring in ring_pos:
            cx = int(self.scale * ring_pos[ring][0] + self.x_offset)
            cy = int(self.scale * ring_pos[ring][1] + self.y_offset)
            cv2.circle(self.clone, (cx, cy), ring_px, (0, 255, 0), -1)
            cv2.circle(self.clone, (cx, cy), ring_px, (0, 0, 0), 1)

        # draw edges thicker so they're visible at high resolution
        for node in self.Nodes:
            ...
            for edge0, edge1 in edges:
                ...
                if edge0 < edge1 and '{:}'.format(edge0) in node_pos and '{:}'.format(edge1) in node_pos:
                    crd0 = (int(self.scale * node_pos['{:}'.format(edge0)][0] + self.x_offset),
                            int(self.scale * node_pos['{:}'.format(edge0)][1] + self.y_offset))
                    crd1 = (int(self.scale * node_pos['{:}'.format(edge1)][0] + self.x_offset),
                            int(self.scale * node_pos['{:}'.format(edge1)][1] + self.y_offset))
                    # line thickness relative to scale (but at least 1 px)
                    thickness = max(1, int(self.scale * 0.02))
                    cv2.line(self.clone, crd0, crd1, (0, 0, 0), thickness)

        # draw any undercoordinated/highlight markers as your original code did (if present)
        try:
            for i in self.undercoordinated:
                key = '{:}'.format(int(i))
                if key in self.nodes:
                    cx = int(self.scale * self.nodes[key]['crds'][0] + self.x_offset)
                    cy = int(self.scale * self.nodes[key]['crds'][1] + self.y_offset)
                    # bigger highlight circle
                    cv2.circle(self.clone, (cx, cy), max(8, int(0.2 * self.scale)), (36, 255, 12), -1)
        except Exception:
            pass

        # finished refreshing clone; returns None
        return
