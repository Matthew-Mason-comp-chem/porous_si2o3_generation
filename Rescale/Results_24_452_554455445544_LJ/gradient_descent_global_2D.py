...
subprocess.run(["/u/mw/jesu2929/lammps-23Jun2022/build/lmp", "-in", file_string+"_temp.in", "-l", file_string+"_temp_log.lammps", "-screen", "debug.txt"])
...
#    phi = np.arange(0,np.pi,np.pi/10)
    #phi = np.array([np.pi/2])
#    theta = np.arange(0,2*np.pi,np.pi/10)
    rho = np.array([0.25*step_size,0.5*step_size, step_size, 2*step_size])
