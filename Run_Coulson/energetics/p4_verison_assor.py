...
for idx, fil in enumerate(tqdm(hamf)):
   ...
   # ham_e = harmonic_e / (int(sys_s))
    ...
    ## Obtaining electro per atom
    #if idx >= len(elef):
    #    continue

    #with open(elef[idx] , 'r') as f:
    #    elec = np.genfromtxt(f)
    #    elec = (elec[2]) / int(sys_s)
    #    #print(elec)
    #    ele_e_tot.append(elec)
    ...
    if os.path.exists(folder_name):
        ...
        # obtaining assor 
        with open(folder_name + '/test_correlations.out') as f:
...
#ele_e_tot = np.array(ele_e_tot)
sys_s_tot = np.array(sys_s_tot)
var0_tot = np.array(var0_tot)
p4_vals = np.array(p4_vals)
assor = np.array(assor)
...
outname = f"assor_vs_p6_{pore_size}"
...
#    from sklearn.linear_model import LinearRegression
#    model = LinearRegression()
#    model.fit(ref_ass.reshape(-1, 1), ref_hamE)
#    xfit = np.linspace(ref_ass.min(), ref_ass.max(), 200)
#    yfit = model.predict(xfit.reshape(-1, 1))
#
#    # plot reference points and regression line
#    plt.scatter(ref_ass, ref_hamE, color='gray', marker='o', label='Reference data')
#    plt.plot(xfit, yfit, color='black', linestyle='--', label='Linear fit')
#
#    print(f"Reference linear fit: y = {model.coef_[0]:.3g} * x + {model.intercept_:.3g}")
#
...
#
#
#outname = f"harmonic_vs_assor_{pore_size}"
#
#if save_fig:
#    plt.savefig(outname, dpi=300)
#    print("Saved figure to", outname)
...
#    print(f"Could not load or fit reference data: {e}")
#
#outname = f"electrostatic_vs_assorr_{pore_size}"
#
#if save_fig:
#    plt.savefig(outname, dpi=300)
#    print("Saved figure to", outname)
