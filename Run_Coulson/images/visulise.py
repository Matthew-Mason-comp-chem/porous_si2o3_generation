...
if not os.path.isdir(input_dir):
    ...
    sys.stderr.write(f"\nERROR: Expected input directory does not exist:\n  {input_dir}\n\n")
    sys.exit(2)

folder_name = os.path.join(input_dir, 'Run')
file_name = 'test'
file_type = 'Si2O3'
...
# plots oringinal crds plt.scatter(o_crds[:,0], o_crds[:,1], color='r', s=0.5)


ax.set_xlim(-pb[0] * 0.5, pb[0] * (NX + 0.5))
ax.set_ylim(-pb[1] * 0.5, pb[1] * (NY + 0.5))

plt.savefig(f"vis_{pore}_{system}_{temp}_{seed}_{steps}.png", dpi=600)
